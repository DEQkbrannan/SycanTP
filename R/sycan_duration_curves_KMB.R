...
## the flow data is already sorted from largest to smallest
...
FDC <- data.frame(
  date = mean_daily_flow[,1],
  POSIXlt = strptime(mean_daily_flow[,1], format = "%m/%d/%Y"), 
  mean_daily_flow = mean_daily_flow[,2],
  percentile= 100*(1 - rank(mean_daily_flow[,2], ties.method = "first")/
                     (length(mean_daily_flow[,2]) + 1)))
head(FDC)
